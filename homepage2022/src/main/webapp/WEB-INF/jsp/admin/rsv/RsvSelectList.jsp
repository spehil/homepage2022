...
<div id="content">
		...
		<div id="bbs_search">
			<form name="frm" method="post" action="/admin/rsv/rsvSelectList.do">
				<fieldset>
					...
					<input name="searchKeyword" value="<c:out value="${searchVO.searchKeyword}"/>" type="text" class="inp_s" id="inp_text" /><span class="bbtn_s"><input type="submit" value="검색" title="검색(수업용 예약관리 게시물  내)" /></span>
				</fieldset>
			</form>
		</div>
	<%--목록영역 --%> 			
	<div id="bbs_wrap">
		...
		<div class="bss_list">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr>
						...
						<td class="tit">
							...
							<td>
								<c:out value="${result.reqstBhnde}"/>~<br/>
								<c:out value="${result.reqstEndde}"/>
							</td>
							<td><c:out value="${result.use.BeginTime}~${result.use.EndTime}"/></td>
			...
			<div id="paging">
				<c:url var="pageUtl" value="/admin/rsv/rsvSelectList.do${_BASE_PARAM}"/>
