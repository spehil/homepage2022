...
public class NaverLoginServiceImpl extends EgovAbstractServiceImpl implements NaverLoginService {
	...
	private final static String SESSTION_STATE = "oauth_state";
	...
	public String getAuthorizationUrl(HttpSession session, String domain) {
		...
		/* 세션 유효성 검증을 위한 난수를 생성 */
		String state = generateRandomString();
		/* 생성한 난수 값을 session에 저장 */
		setSession(session, state);

		/* Scribe 에서 제공하는 인증URL 생성 기능을 이용하여 네아로 인증 URL 생성 */
	...
	public void setSession(HttpSession session, String state) {
		session.setAttribute(SESSTION_STATE, state);
	}

	/* http session에서 데이터 가져오기 */
	public String getSession(HttpSession session) {
		return (String) session.getAttribute(SESSTION_STATE);
	}
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		...
		Request request = new Request.Builder() // okhttp3.Request
				...
				.addHeader("Authorization", "Bearer " + oauthToken.getAccessToken()).build();
		Response response = client.newCall(request).execute();

		return response.body().string();//Json으로 받은걸  다시 string으로
	}
}
